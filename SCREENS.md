...
3. **Before setup**, the workspace tabs (Freelancers, Invoices, Time) show embedded **empty states** ("No freelancers yet", etc.) with a **"Your account's almost ready → Set up account"** banner; after setup, those tabs instead navigate to the populated standalone pages.
4. The "Start" button on the active checklist step launches the **setup-steps wizard** — [`client-onboarding.html`](client-onboarding.html) ("Set up your account"), whose steps are: 1) Tell us about yourself, 2) People behind your business, 3) Provide your documents, 4) Tell us about your work, 5) Compliance declaration, 6) Review & confirm (+ SMS OTP). On completion the home switches to the **after-setup** state (`dashboard.html`).
